...
def build(embed: bool) -> str:
    ...
    if "enc_r" in n:
        ...
        enc_txt = f"""<p>Ridge regression from layer features to each cell's 54–63-image tuning curve reaches a mean cross-validated <i>r</i> of <b>{n['enc_r']:.2f}</b> for MTL concept cells at the best layer ({n['enc_best']}; {n['enc_frac']:.0f}% of cells above zero; ≈{n['enc_ceiling']:.2f} of the split-half ceiling), versus {n['enc_cat']:.2f} for a category one-hot baseline. Raw CV <i>r</i> is <em>negatively</em> biased for signal-free targets with ~60 samples, so we also computed a per-cell picture-label-shuffle null (100 shuffles). Against that null the defensible comparison is the fraction of cells individually significant (subtracting predictor-specific null means would re-order models by their bias): CLIP's last layer 47%, DINOv2 40%, AlexNet fc6 39%, ResNet-50 34%, VGG fc7 34%, category one-hot 32% — CLIP is the best single-cell predictor, and MFC concept cells are predicted less well (38% for CLIP, others 21–31%).</p>"""
...
<div class="prose"><p>Two further checks make the "patient × picture" point. Concept cells in different patients that prefer the <em>same</em> picture agree on the rest of their tuning no better than chance (ρ = 0.04, 95% CI −0.03…0.10, n = 43 pairs), and the same-category vs different-category difference disappears at the patient-pair level (57 pairs, p = 0.47): tuning does not transfer across people. And a leave-one-subject-out classifier trained on other patients cannot predict which pictures a held-out patient's concept cells will prefer from any feature set (AUC 0.44–0.49). Category itself is decodable from single-trial MTL population activity, but only weakly (+4.5% over chance; MFC and hippocampus at chance).</p></div>
<div class="grid2">
...
<div class="prose">
<p>For each screening session we built the MTL and MFC population RDM (correlation distance over z-scored unit responses, 54–63 pictures) and compared it with layer-wise RDMs from AlexNet, VGG-16, ResNet-18/50, ConvNeXt-T, ViT-B/16, DINOv2-S and CLIP ViT-B/32. In every architecture the MTL correlation rises from early to mid/late layers — monotonic to the last layer for ResNet-50 and CLIP, plateauing from mid-depth for AlexNet, VGG and DINOv2 (best: {n['rsa_best']}, ρ = {n['rsa_best_rho']:.3f}, p = {n['rsa_best_p']:.3f} across 18 sessions; category RDM alone {n['rsa_cat']:.3f} with hand-corrected labels; low-level 0.014). The MFC is weak, near {n['rsa_mfc']:.3f}, and without a depth gradient; MTL &gt; MFC is decisive as a fixed effect (label permutation z ≈ 6) but marginal across patients (paired p 0.01–0.19 by layer, mixed model p = 0.06). Split-half reliability of an MTL RDM is only ≈0.06, so ceiling-normalised ρ is ≈0.24 (ratio of means). Amygdala (0.05–0.06) ≫ hippocampus (0.015); hippocampal geometry is barely categorical (ρ = 0.008 with the category RDM) and weakly captured by late layers.</p>
<p>Because the picture pool is shared, per-session RDMs can be rank-normalised and pooled over co-shown pairs into a consensus 342×342 MTL RDM. Against it the best layer reaches ρ = {n['pooled_best_rho']:.3f} (permutation p = 0.005, ≈10 null SDs), category {n['pooled_cat']:.3f}. Partialling out the category and low-level RDMs leaves CLIP's last layer significant (partial ρ = {n['part_clip_rho']:.3f}, p = {n['part_clip_p']:.3f}; ResNet-50 avgpool {n['part_rn_rho']:.3f}, p = {n['part_rn_p']:.3f}), — late layers and the category RDM are largely redundant, and late layers add a small unique component. Restricted to <em>within</em>-category pairs, ViT-family late layers still track the MTL (CLIP ln_post 0.052, p = 0.015; ImageNet ViT-B/16 0.030, p = 0.016) whereas ResNet-50 (−0.006) and DINOv2 do not. In time, the correspondence rises from ~200 ms and peaks at <b>{n['t_peak']} ms</b> after picture onset, with late layers &gt; category &gt; early layers &gt; low-level throughout.</p>
</div>
...
<div class="prose">
<p>Take a concept cell, drop its preferred picture, and ask whether its firing to the remaining 53–62 pictures follows their similarity to that preferred picture in a given layer. It does: cell-pooled Spearman ρ reaches <b>{n['sim_rho']:.2f}</b> at {n['sim_best']} with {n['sim_frac']:.0f}% of MTL concept cells positive, against a random-anchor null of ≈0. Cells are nested in patients (two patients hold 42% of them), so the honest effect size is the patient mean, 0.10 (Wilcoxon over 16 patients p = 0.016; cluster-robust p = 2×10<sup>−4</sup>), and 0.12 for selection-corrected strict cells (n = 67). The effect grows with layer depth (mixed model z = 14), survives within category (0.11; category-partialled 0.09; AlexNet conv1 ≈ 0) and beyond CLIP text-space similarity; MFC concept cells show it only at late layers; non-selective MTL cells barely at all. Right-amygdala cells generalise far more than left (0.30 vs 0.04; patient-level p = 0.008; hemisphere partly confounded with patient).</p>
{enc_txt}
</div>
...
<div class="prose">
<p>With only five memoranda per patient there is little similarity variance to work with. There is no reliable behavioural effect: the best of 30 tests (5 spaces × 3 conditions × 2 outcomes) is a CLIP lure-similarity slowing on correct OUT trials (mean within-subject slope +0.055 z-units, p = 0.04 uncorrected, ≈0.8 such hits expected under the global null); other spaces and outcomes are null.</p>
</div>
...
<div class="prose">
<ul class="findings">
<li>An adversarial statistical review (docs/review_2026-08-15.md) led to a strict, selection-corrected concept-cell criterion carried alongside the paper's, patient-level / cluster-robust tests next to cell-pooled ones (A8_cluster_robust.csv), family-wide FDR for the memoranda contrasts, and softer wording throughout; two earlier claims (object models &gt; VGGFace2 for face cells; category-level tuning transfers across patients) did not survive patient-level tests and are withdrawn.</li>
