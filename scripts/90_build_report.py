...
<p>The five Sternberg memoranda per patient were chosen online as the pictures that drove the most selective neurons. Nothing about a picture <em>by itself</em> distinguishes them: no category is enriched (faces OR 1.4, n.s.), no CLIP attribute, no low-level statistic and no DNN-space descriptor (typicality, isolation, distance to centroid) separates memoranda from the rest of a patient's set (all FDR q &gt; 0.2). Our own replicated neural selectivity does separate them (within-subject AUC 0.67–0.71) — but a picture that "won" in <em>other</em> patients is barely more likely to win in this one (leave-one-subject-out AUC 0.54).</p>
